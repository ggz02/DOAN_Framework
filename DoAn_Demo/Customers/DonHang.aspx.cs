...
namespace DoAn_Demo
{
    public partial class DonHang : System.Web.UI.Page
    {
        ...
        protected void LoadData()
        {
            ...
            dtgvDonHang.Columns[4].ControlStyle.Width = 100;
        ...
        protected void butDongY_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                adapt.InsertCommand = new SqlCommand("INSERT INTO DDH VALUES (@MaDH, @MaKH, @NgayDH, @TriGia, @TenNguoiNhan, @DiaChiNhan, @SoDT, @HThanhToan)", con);

                string query = "select top (1) MaDH from DDH order by MaDH desc";
                ...
                if (!string.IsNullOrEmpty(currentMaDH))
                {
                    ...
                    string numberPart = currentMaDH.Substring(3); // Lấy phần số sau chuỗi "DDH"
                    int number = int.Parse(numberPart);
                    number++;

                    // Tạo chuỗi mới MaDH
                    newMaDH = "DDH" + number.ToString("00");
                ...
                SqlCommand cmdInsertCTDDH = new SqlCommand("INSERT INTO CTDDH VALUES (@MaDH, @MaSP, @DonGia, @SoLuong, @ThanhTien)", con);
                foreach (DataRow r in cart.Rows)
                {
                    ...
                    // Remove formatting characters from GIA column
